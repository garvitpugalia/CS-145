...
class KMeans:
    ...
    def reassignClusters(self, dataSet, c, clusters):
        ...
        for point in dataSet:
            ...
            # assign point to the closest cluster by adding to appropriate index
    ...
    def getCentroid(self, cluster):
        ...
        size = len(cluster)
        # ****************Please Fill Missing Lines Here*****************
        # for each point in cluster, sum up x and y values
        for point in cluster:
            cx = cx + point.x
            cy = cy + point.y
        # divide by number of points in cluster to get centroid
        cx = cx / float(size)
        cy = cy / float(size)
