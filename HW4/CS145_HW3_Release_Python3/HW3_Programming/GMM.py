...
class GMM:
    ...
    def Estep(self):
        for i in range(len(self.dataSet)):
            ...
            for j in range(self.K):
                # for each (i,j), divide by denominator=sum(W[i]) to compute probabilities
                prob = self.W[i][j] / denominator
                self.W[i][j] = prob
    ...
    def Mstep(self, clusters):
        for j in range(self.K):
            ...
            for i in range(len(self.dataSet)):
                ...
                # cov_xy +=?
                # ****************Please Fill Missing Lines Here*****************
                # covariance of xy is product of (y - y_mean) * (x - x_mean)
                cov_xy += self.W[i][j] * (self.dataSet[i].y - self.mean[j][1]) * (self.dataSet[i].x - self.mean[j][0])
                # all updates are weighted by W[i][j]
            ...
            # update w[j]
            # ****************Please Fill Missing Lines Here*****************
            # w[j] = sum(W[i][j]) for all i in dataset, which is equal to denominator
