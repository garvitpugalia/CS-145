...
def getBeta(train_x, train_y):   
    ...
    y_dot_product = np.dot(train_x_transpose, train_y)
    x_dot_product = np.dot(train_x_transpose, train_x)
    inverse_x_dot_product = np.linalg.inv(x_dot_product)

    beta = np.dot(inverse_x_dot_product, y_dot_product)
    return beta
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
       ...
       for i in range(n):
           ########## Please Fill Missing Lines Here ##########
           x_beta_transpose = np.dot(train_x[i].transpose(), beta)
           deriv = deriv + np.dot(train_x[i], x_beta_transpose - train_y[i])
       deriv = deriv / n
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            ########## Please Fill Missing Lines Here ##########
            x_beta_transpose = np.dot(train_x[idx].transpose(), beta)
            difference = train_y[idx] - x_beta_transpose
            val = np.dot(difference, train_x[idx])
            beta = beta + val.dot(lr)
    return beta
