...
def compute_avglogL(X,y,beta):
    ...
    log_loss = 0
    for idx in range(n):
        x_beta_transpose = np.dot(X[idx].transpose(), beta)
        log_x_beta_transpose = np.log(1 + np.exp(x_beta_transpose))
        log_loss = log_loss - ((y[idx] * x_beta_transpose) - log_x_beta_transpose)
    avglogL = log_loss/n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ########## Please Fill Missing Lines Here ##########
        h = sigmoid(np.dot(train_x, beta))
        descent = np.dot(train_x.transpose(), h - train_y)
        beta = beta - descent.dot(lr)
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        h = sigmoid(np.dot(train_x, beta))
        descent = np.dot(train_x.transpose(), train_y - h)
        
        #(post 50 on Piazza)
        hessian_matrix = np.zeros((n,n))
        hessian_matrix = -np.dot(train_x.transpose() * h, train_x)

        beta = beta - (np.dot(np.linalg.inv(hessian_matrix), descent) / n)
...
########## Please Fill Missing Lines Here ##########
# train_x and train_y are numpy arrays
# lr (learning rate), and reg are scalars
# function returns value of beta calculated using (0') batch gradient descent with regularization
def getBeta_RegularBatchGradient(train_x, train_y, lr, num_iter, verbose, reg = 0.005):
    beta = np.random.rand(train_x.shape[1])
    ...
    for iter in range(0, num_iter):
        ...
        descent = np.dot(train_x.transpose(), h - train_y)
        for j in range(0, p):
            descent[j] = (descent[j] / n) + 2 * reg * beta[j]
        beta = beta - descent.dot(lr)
        if(verbose == True and iter % 1000 == 0):
            avgLogL = compute_avglogL(train_x, train_y, beta)
            print(f'average logL for iteration {iter}: {avgLogL} \t')
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getBeta_RegularBatchGradient(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose)
            print('Beta: ', beta)
        else:
