...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        a2 = np.maximum(0, h1) # activation with input of h1
        h2 = a2.dot(W2.T) + b2
        scores = h2
        ...
        def softmax_loss(x, y):
            ...
            # convert hot vectors into actual form
            y_labels = []
            for i in y:
                y_label_curr = []
                for j in range(C):
                    if j == i:
                        y_label_curr.append(1)
                    else:
                        y_label_curr.append(0)
                y_labels.append(y_label_curr)

            # apply softmax function to the x input
            if x.ndim == 2:
                x_vals = x.T - np.max(x.T, axis=0)
                y_pred = np.exp(x_vals) / np.sum(np.exp(x_vals), axis=0)
                y_pred = y_pred.T
            else:
                x_vals = x - np.max(x)
                y_pred = np.exp(x_vals) / np.sum(np.exp(x_vals))

            dx = y_pred - y_labels
            log_vals = np.log(y_pred)
            loss = -np.sum(log_vals) / N
        ...
        def MSE_loss(y_pred, y_target):
            ...
            dy_pred = (y_pred - y_labels) / N

            errors = []
            for i in range(N):
                error = 0.0
                for j in range(C):
                    error += (y_pred[i][j] - y_labels[i][j]) * (y_pred[i][j] - y_labels[i][j])
                errors.append(error)

            total_error = np.sum(errors)
            loss = total_error / (2 * N)
