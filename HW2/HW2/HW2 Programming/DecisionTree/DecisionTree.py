...
def compute_info_gain(data, att_name, y_name):
    ...
    for index, attr_val in enumerate(vals):
        ratio = float(counts[index])/float(total_counts)
        attr_data = data[data[att_name] == attr_val][y_name]
        conditional_entropy += ratio * float(compute_entropy(attr_data))
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    att_info = 0.0
    ########## Please Fill Missing Lines Here ##########
    for index, attr_val in enumerate(values):
        attr_val_ratio = float(counts[index])/float(total_counts)
        att_info -= attr_val_ratio * np.log2(attr_val_ratio)
...
class DecisionTree(object):
    ...
    def make_tree(self, data, parent_node_class = None):
        ...
        if len(np.unique(data[self.class_name])) <= 1:
            ...
            ########## Please Fill Missing Lines Here ##########
            if len(data) != 0:
                leaf_value = np.unique(data[self.class_name])
            return leaf_value
